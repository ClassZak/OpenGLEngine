...
public:
	...
	{
		this->m_vertexes = GenerateOldCircleVertexes(count, radius, center);
		this->m_indexes = GenerateOldCircleVertexIndexes(count);
	...
	(
		...
		unsigned int VBO = 0u,
		unsigned int IBO = 0u
	) : OldCircle(count, radius, center, VAO, VBO, IBO)
	{
...
public:
	...
	static std::vector<Vertex2D<T>> GenerateOldCircleVertexes
	...
	/// </summary>
	/// <param name="OldCirclePointCount">Число точек</param>
	/// <returns>Индексы для отрисовки</returns>
	static std::vector<unsigned int> GenerateOldCircleVertexIndexes(unsigned int OldCirclePointCount)
	{
