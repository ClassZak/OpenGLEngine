...
template<typename T>
class CircleSector : public AShape<Vertex2D<T>>
{
public:
	CircleSector
	(
		...
		unsigned int VBO = 0u,
		unsigned int IBO = 0u
	) : CircleSector(count, radius, center, startDegree, endDegree, VAO, VBO, IBO)
	{
	...
	{
		this->m_vertexes = GenerateCircleSectorVertexes(count, radius, center, startDegree, endDegree);
		this->m_vertexes.insert(this->m_vertexes.begin(), center);
		this->m_indexes = GenerateCircleSectorVertexIndexes(count);
...
template<class T>
inline void CircleSector<T>::Init()
{
	if (!this->m_VAO)
...
template<class T>
inline void CircleSector<T>::Draw()
{
	GLLogCall(glBindVertexArray(this->m_VAO));
	GLLogCall(glBindBuffer(GL_ARRAY_BUFFER, this->m_VBO));
